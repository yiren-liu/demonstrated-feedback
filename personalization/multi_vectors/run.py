...
from multi_vectors.pipeline.cluster import run_clustering
...
def run_clustering_step(dataset, algorithm, pca, cluster_embedding_model, min_cluster_size, output_dir, logger):
    ...
    logger.info(f"Apply PCA: {pca}")
    logger.info(f"Min cluster size: {min_cluster_size}")
    logger.info(f"Cluster embedding model: {cluster_embedding_model}")

    clusters_filename = run_clustering(
        dataset=dataset,
        algorithm=algorithm,
        pca=pca,
...
def main():
    ...
    parser.add_argument("--cluster-embedding-model", type=str, default="AnnaWegmann/Style-Embedding",
                        help="Embedding model for clustering prompts (default: AnnaWegmann/Style-Embedding)")
