...
def score_with_rouge_l(
    ...
    predictions = [entry["prediction"].replace('\n', ' ') for entry in entries]
    ...
    with Progress(*progress_columns) as progress:
        ...
        for idx, (reference, prediction, entry) in enumerate(
            zip(references, predictions, entries), start=1
        ):
            ...
            # Preserve all original columns from entry
            result_row = dict(entry)  # Copy all original columns
            # Add ROUGE metrics
            result_row.update({
...
def compute_rouge_metrics(csv_path: Path) -> None:
    ...
    # Filter out avg row if present
    entries = [row for row in rows if row.get("index", "").lower() != "avg"]
    ...
    save_rouge_results_to_csv(
        ...
        fieldnames=list(scored_rows[0].keys()) if scored_rows else None,
    )
