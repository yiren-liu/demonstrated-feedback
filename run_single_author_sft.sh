#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────────
# Train (SFT only) + generate for a single (condition, author) pair.
#
# Usage:
#   bash run_single_author_sft.sh --condition seen --seed 0 --author 3
#   bash run_single_author_sft.sh --condition unseen-genre --genre b --author 7
#   bash run_single_author_sft.sh --condition seen --seed 0 --author 3 --dry
# ──────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
GEN_GPU="${GEN_GPU:-0}"
BASE_OUTPUT=outputs/genre_holdout_sft_exp
...
case "${CONDITION}" in
    ...
    single-genre)
        ...
        BASE_OUTPUT=outputs/single_genre_sft_exp
        ;;
    *)
...
echo ""
echo "============================================================"
echo "  SFT-ONLY  ${CONDITION^^}  tag=${TAG}  -> ${OUT_DIR}"
echo "============================================================"
...
run_cmd CUDA_VISIBLE_DEVICES="${TRAIN_GPU}" python scripts/run_sft_only.py "${CONFIG}" \
    --train_pkl="${TRAIN_PKL}" \
